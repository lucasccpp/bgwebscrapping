...
def get_calendar(dt_start, dt_end):
    ...
    dim_calendarTemp = pd.DataFrame()
    dim_calendar = pd.DataFrame()
    dim_calendarTemp["data"] = pd.date_range(start=dt_start, end=dt_end)
    dim_calendarTemp["sk_data"] = dim_calendarTemp.data.apply(lambda x: x.strftime("%Y") + x.strftime("%m") + x.strftime("%d"))
    dim_calendarTemp["pk_data"] = dim_calendarTemp.data.apply(lambda x: x.strftime("%Y") + x.strftime("%m") + x.strftime("%d"))
    dim_calendar["sk_data"] = dim_calendarTemp["sk_data"]
    dim_calendar["pk_data"] = dim_calendarTemp["pk_data"]
    dim_calendar["data"] = dim_calendarTemp["data"]
...
def main():
    try:
        ...
        dim_calendario.to_csv("998-baseDados/dim_calendario.csv", sep=';', encoding='utf-8', index=False)
